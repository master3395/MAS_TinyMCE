<h3>Versions for CMS Made Simple 2.2.15+ with TinyMCE 8</h3>
<p>Official upstream packages and their release dates are listed on the CMS Made Simple Forge: <a href="https://dev.cmsmadesimple.org/project/files/12" target="_blank" rel="noopener noreferrer">TinyMCE (Forge files)</a>.</p>
<ul>
  <li>
    <p>
      <strong>Version 8.0.3</strong> (site maintenance, <strong>13.04.2026</strong>)
      <br><br>
      Upgraded the bundled TinyMCE core from the 8.3.2 line to <strong>8.4.0</strong> (GPL core). Updated editor assets, plugins, skins and notices (including DOMPurify 3.3.2), while preserving CMS Made Simple integrations (<em>cmsms_filepicker</em>, <em>cmsms_linker</em>, <em>cmsms_template</em>) and profile behavior. Added a safe default for <code>allow_noneditable</code> in the stock init template, and kept <code>extra_js</code> as the override path for advanced TinyMCE options. Existing FilePicker post-upload image resize behavior remains active through <code>FileManager::OnFileUploaded</code>. Added two optional CMSMS external plugins, <code>cmsms_imageedit</code> and <code>cmsms_markdown</code>, both disabled by default and enabled per profile. Targets PHP 7.4 through 8.5.
    </p>
    <br>
  </li>
  <li>
    <p>
      <strong>Version 8.0.2</strong> (site maintenance, <strong>12.04.2026</strong>)
      <br><br>
      Profile normalization for file manager flags; <em>cmsms_filepicker</em> and file-picker types in the generated init for image and link dialogs; file picker URL handling in the module class; TinyMCE JS cache refresh on upgrade; configurable <em>admin submenu</em> (top-level admin section; save clears caches). Removed the bundled Responsive File Manager from the package: admin browsing and uploads use the CMS Made Simple FilePicker and the <em>cmsms_filepicker</em> plugin only; profiles store <code>filemanager_use_responsive_fm = 0</code>; legacy <code>responsivefilemanager</code> tokens are stripped from toolbar and plugin lists; obsolete <code>filemanager_salt</code> preference removed. Per-TinyMCE profile options for automatic server-side image resize after upload (File manager tab): limits apply to FilePicker uploads from the editor for that profile; the module registers limits against the FilePicker session signature and scales images in place (JPEG/PNG/GIF/WebP where GD supports them) via the core <code>FileManager::OnFileUploaded</code> event; FilePicker invokes that hook on every upload so listeners still run when profile thumbnails are disabled. Targets PHP 7.4 through 8.5.
    </p>
    <br>
  </li>
  <li>
    <p>
      <strong>Version 4.0.0-beta3</strong> (Forge release <strong>25.02.2026</strong>)
      <br>
      <ul>
        <li>
          Bug fixes wrt backward compatibility with PHP7.4
        </li>
        <li>
          Updated to Tiny(MCE) 8.3.2
        </li>
        <li>
          Fixed missing languages files
        </li>
      </ul>
    </p>
    <br>
  ...
  <li>
    <p>
      <strong>Version 4 (4.0.0-beta1)</strong> (Forge release <strong>17.09.2025</strong>)
      <br>
      The TinyMCE module dev team is happy to release this new major version!
      <br>
      <br>
      As this version is a major upgrade, the upgrade tool is going to add and delete files and DB columns. <strong>We strongly encourage website administrators to <u>carefully test it on a copy of your website before running it in production</u></strong>.
      <br>
      <br>
      You may have to adapt your own JS loading script (if you're using a custom one).
      <ul>
        <li><strong>Updated Tiny(MCE) to 8.1.0</strong></li>
        <li>Working in PHP8.3 - Work in progress, for 8.4 too</li>
        <li><strong>Removed <em>Responsive File Manager</em></strong> as it is not maintained anymore (thanks Alberto, the developer, for this component we were using for the last years)</li>
        <li>TinyMCE <strong>now uses the CMSMS FilePicker plugin</strong> and module to handle folder creation, file upload and selection - You can select the FilePicker profile you want to use with each TinyMCE profile!</li>
        <li>
          Removed plugins:
          <ul>
            <li>paste - now included in the TinyMCE core</li>
            <li>imagetools - is now a premium TinyMCE plugin</li>
            <li>template - is now a premium TinyMCE plugin.. but we developed our own plugin - see below</li>
          </ul>
          New plugins:
          <ul>
            <li>cmsms_template - replaces the old template plugin - Selects a DesignManager template to be included in the WYSIWYG content</li>
            <li>accordion - new plugin in TinyMCE</li>
          </ul>
        </li>
        <li>
          Replaced <em>formatselect</em> toolbar button with <em>blocks</em>
        </li>
        <li>
          ...
          <ul>
            <li>Added "license_key", "promotion" and "onboarding" option - Default configuration uses GPL license, but you can switch to a commercial license too</li>
            <li>"templates" option is replaced by "cmsms_templates"</li>
            <li>Added "newline_behaviour" and adapted "forced_root_block"</li>
          </ul>
        </li>
        <li>
